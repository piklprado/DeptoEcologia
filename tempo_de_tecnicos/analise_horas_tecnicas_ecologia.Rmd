...
```{r tabela}
prop.sumario[,2:4] <- prop.sumario[,2:4]*100
kable(prop.sumario, col.names=c("Responsável", "Básico", "Médio", "Superior"), digits=1,
...
superior <- prop.sumario[order(prop.sumario$S, decreasing=TRUE),c(1,4)] %>%
    filter(S>0)
par(mfrow=c(1,3), mar=c(5, 5, 4, 10))
...
barplot(as.matrix(medio[,2]), legend.text=medio[,1], col=cores[medio[,1]],
        args.legend=list( x=5.2, y= 105 , bty = "n"), main="Médio, 2011 - 2017", ylab= "")
barplot(as.matrix(superior[,2]), legend.text=superior[,1], col=cores[superior[,1]],
        args.legend=list( x=5.2, y= 105 , bty = "n"), main="Superior, 2011 - 2017", ylab= "")
par(mfrow=c(1,1))
```
